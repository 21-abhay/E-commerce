...
const viewProfile = (req, res) => {
    ...
    if (req.session.user == profileid) {
        // let cmd = `SELECT * FROM abhay_users WHERE user_id='${profileid}'`;
        ...
        con.query(cmd, (err, results) => {
            if (err) throw err;
            // console.log("Date of Birst : ",results[0].dob)
            // results[0].dob = results[0].dob.toString().replace('T',' ');
            // console.log("Date of Birst : ",results[0].dob)
            // results[0].dob = results[0].dob.toString().replace('T',' ').split('T');
            // console.log("Date of Birst : ",results[0].dob)
            ...
            // cmd = `SELECT * FROM books WHERE Upload_by='${req.session.user}'`;
